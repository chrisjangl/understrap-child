 ...
 * Navigation
 *
 * register navigation menus to be used throughout the theme
 ** 
 ** 
 *
 * @version     1.0
 ...
 */
 ...
 register_nav_menus(array(
     'top_menu' =>  'Top Menu',
     'footer_1' =>  'Footer (left)',
     'footer_3' =>  'Footer (right)',
 ));

 /**
  * create static primary navigation for header instead of using WP's menus.
  * header is brought from the JS site
  *
  * @return string html
  */
function dc_get_navigation_from_JS() {
        ...
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a href="<?php echo $main_site_url; ?>/cart" class="nav-link">
					...
					Cart
                    <?php
					// @todo: how can we handle the cart size?
					// Cart <% if (cartSize > 0) { %> (<%= cartSize %>) <% } %>
					?>
                </a>
            </li>
...
/**
 * print the bar below primary nav with contact info
 *
 * @return string html 
 */
function dc_get_header_contact_bar() {
 ...
 * Overwrites the same class definition in Parent Theme, so we can create end_el function
 *
...
class Understrap_WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_output .= isset( $args->link_before ) ? $args->link_before . $icon_html . '<span class="label">' . $title . '</span>' . $args->link_after : '';
		/**
	...
	public function end_el( &$output, $item, $depth = 0, $args = array() ) {
		...
		if ( $item->title == 'Search' ) {
			...
			// append a search form, built elsewhere
			$output .= dc_get_search_form();

			// enqueue the necessary JS to toggle display of search form
